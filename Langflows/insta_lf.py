...
def analyze_engagement(data):
    data['engagement_rate'] = (data['likes'] + data['comments']) / data['followers'] * 100
    return data

def generate_insights(data):
    avg_engagement = data['engagement_rate'].mean()
    high_engagement_posts = data[data['engagement_rate'] > 10]
    return {
...
result = flow.run({"file_path": "path_to_instagram_scraped_data.csv"})
